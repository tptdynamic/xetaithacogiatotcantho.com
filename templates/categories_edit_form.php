<div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-white" style="flex: 1!important;">
        ...
        <form id="editCategoryForm" action="edit_category" method="POST">
            <div class="row mx-5">
                <label for="inputCategory" class="form-label text-black">Tên chuyên mục</label>
                <input class="form-control" name="category_name" type="text" id="inputCategory" value="<?= $category->TenChuyenMuc ?>">
            </div>
            <div class="row mx-5">
                <label for="inputCategoryDescription" class="form-label text-black">Mô tả</label>
                <textarea name="category_description" type="text" id="inputCategoryDescription" rows="10"><?= $category->MoTa ?></textarea>
            </div>
            <div class="row mx-5">
                <label for="inputSubCategory" class="form-label text-black">Chọn chuyên mục chính</label>
                <select class="form-control" name="main_category" type="text" id="inputSubCategory">
                    <option value="0">Không có</option>
                    <?php
                        foreach ($options as $arr) {
                            echo '<option value="' . $arr->ID . '" ' . (($category->ChuyenMucChinh == $arr->ID) ? "selected" : "") . '>' . $arr->TenChuyenMuc . '</option>';
                        }
                    ?>
                </select>
            </div>
            <div class="row m-5">
                <input type="button" name="path" id="inputThumbnail" name="thumbnail" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#getImageModal" value="<?= $category->AnhDaiDien ?>" onclick="$('#getImagesModal').submit()"></input>
            </div>
            <input class="d-none" name="id" value="<?= $category->ID ?>">
...
<script>
    $edit_form = new Form("#editCategoryForm");
