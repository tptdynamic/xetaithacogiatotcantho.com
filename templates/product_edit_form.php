<div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-white" style="flex: 1!important;">
        ...
        <form id="editProductForm" action="edit_product" method="POST">
            <div class="row mx-5">
                ...
                <input class="form-control" name="product_name" type="text" id="inputProduct" value="<?= $product->TenSanPham ?>">
            </div>
            <div class="row mx-5">
                <label for="inputProductSpecification" class="form-label text-black">Thông số kĩ thuật</label>
                <textarea name="product_specification" type="text" id="inputProductSpecification" rows="10"><?= $product->ThongSo ?></textarea>
            </div>
            <div class="row mx-5">
                <label for="inputProductDescription" class="form-label text-black">Mô tả</label>
                <textarea name="product_description" type="text" id="inputProductDescription" rows="10"><?= $product->MoTa ?></textarea>
            </div>
            <div class="row mx-5">
                <label for="inputProductPrice" class="form-label text-black">Giá tiền</label>
                <input name="product_price" type="number" class="form-control" id="inputProductPrice" value="<?= $product->GiaTien ?>"></input>
            </div>
            <div class="row mx-5">
                <label for="inputProductSale" class="form-label text-black">Giảm giá</label>
                <input name="product_sale" type="number" class="form-control" id="inputProductSale" value="<?= $product->GiamGia ?>"></input>
            </div>
            <div class="row mx-5">
                ...
                <select name="category" class="form-select" aria-label="Default select example">
                    <?php
                        ...
                        foreach ($categories as $arr) {
                            echo '<option value="' . $arr->ID . '" ' . (($product->ChuyenMuc == $arr->ID) ? "selected" : "") . '>' . $arr->TenChuyenMuc . '</option>';
                        }
            ...
            <div class="row m-5">
                <input type="button" id="inputThumbnail" name="thumbnail" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#getImageModal" value="<?= $product->AnhDaiDien ?>" onclick="$button_id='inputThumbnail';$('#getImagesModal').submit()"></input>
            </div>
            <div class="row m-5" id="inputSlideImage">
                <?php
                    $split = explode(", ", $product->BoSuuTapAnh);
                    for ($i = 0; $i < count($split) - 1; $i++) {
                        echo '<input type="button" name="slideshow" class="button-add-image col-md-2" data-bs-toggle="modal" data-bs-target="#getImageModal" value="' . $split[$i] . '" style="background-image: url(\'' . Config::Domain . $split[$i] .  '\');" onclick="$button_id=\'inputSlideImage\'; $button_index = ' . $i . ';$(\'#getImagesModal\').submit()">';
                    }
                ?>
                <input type="button" name="slideshow" class="button-add-image col-md-2" data-bs-toggle="modal" data-bs-target="#getImageModal" onclick="$button_id='inputSlideImage'; $button_index = <?= count($split) - 1 ?>;$('#getImagesModal').submit()">
            </div>
...
<script>
    $add_form = new Form("#editProductForm");
